...
void GBufferRenderer::constructFBO()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, Engine::config.width / Engine::config.resolutionScale, Engine::config.height / Engine::config.resolutionScale, 0, GL_RGB, GL_FLOAT, NULL);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA2, Engine::config.width / Engine::config.resolutionScale, Engine::config.height / Engine::config.resolutionScale, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, Engine::config.width / Engine::config.resolutionScale, Engine::config.height / Engine::config.resolutionScale);
